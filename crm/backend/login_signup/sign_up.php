...
<?php
    ...
    if (checkReq($_POST)) {
        ...
        if ($no_of_row>0){
            ...
            $stmt="INSERT INTO `users` (name,email,phone,password,verification_code,created_at) VALUES (?,?,?,?,?,?)";
            $sql=mysqli_prepare($conn, $stmt);
        
            //binding the parameters to prepard statement
            mysqli_stmt_bind_param($sql,"ssisss",$business_details["name"],$business_details["email"],$business_details["phone"],$pass,$code,$created_at);
            ...
            if ($result) {
                    ...
                    echo "<script>
                                alert('Successfully account created. Please verify your email.');
